...
// tools/gen-brand.ts
var gen_brand_exports = {};
__export(gen_brand_exports, {
  BRAND: () => BRAND,
});
module.exports = __toCommonJS(gen_brand_exports);
var import_node_fs = require('node:fs');
var import_node_path = require('node:path');
var import_node_module = require('node:module');
var import_meta = {};
var require2 = (0, import_node_module.createRequire)(import_meta.url);
var _sharp = require2('sharp');
var sharp = typeof _sharp === 'function' ? _sharp : _sharp.default;
var OUT = (0, import_node_path.resolve)(import_meta.dirname, '../apps/client/src/assets');
var PWA_ICONS = (0, import_node_path.resolve)(import_meta.dirname, '../apps/client/pwa/icons');
var RESOURCES = (0, import_node_path.resolve)(import_meta.dirname, '../resources');
var BRAND = {
...
var FONT = "'Trebuchet MS','DejaVu Sans',Verdana,sans-serif";
...
function linear(id, from, to, angle = 90) {
...
function prism(cx, cy, r, rot = 0, opacity = 1) {
  const pts = (a, b) => {
    const p = (deg) => {
...
function shard(x, y, w, h, fill, skew = 0.22) {
  const s = w * skew;
  ...
  icon_gear: () =>
    svg(
      ...
      `<g transform="translate(80,60)">
        ${[...Array(8)].map((_, i) => `<rect x="-9" y="-52" width="18" height="24" rx="4" fill="${BRAND.text}" transform="rotate(${i * 45})"/>`).join('')}
  ...
  icon_cards_fan: () =>
    svg(
      ...
      `<g transform="translate(220,150)">
        ${[-28, -14, 0, 14, 28]
          .map(
            (a, i) => `<g transform="rotate(${a}) translate(0,-18)">
...
function avatar(w, h, shardIndex) {
...
function modeTile(w, h, shardIndex, kind) {
  const cx = w / 2;
  const cy = h / 2;
  ...
  const art = {
    ...
    wild: `${[0, 1, 2, 3]
      .map(
        (i) => `<circle cx="${cx + Math.cos((i * Math.PI) / 2) * r * 0.42}"
                   cy="${cy + Math.sin((i * Math.PI) / 2) * r * 0.42}"
                   r="${r * 0.5}" fill="${BRAND.shard[i]}" opacity="0.75"/>`,
      )
      .join('')}`,
    free: `${[0, 1, 2, 3].map((i) => shard(cx - r + i * (r / 2), cy - r, r * 0.42, r * 2, BRAND.shard[i])).join('')}`,
  };
  return svg(
    w,
    h,
...
async function main() {
  ...
  (0, import_node_fs.mkdirSync)(PWA_ICONS, { recursive: true });
  (0, import_node_fs.mkdirSync)(RESOURCES, { recursive: true });
  let n = 0;
  for (const [name, make] of Object.entries(A)) {
    await render(make(), (0, import_node_path.join)(OUT, `${name}.png`));
    n += 1;
  }
  await render(appIcon(192, false), (0, import_node_path.join)(PWA_ICONS, 'icon-192.png'));
  await render(appIcon(512, false), (0, import_node_path.join)(PWA_ICONS, 'icon-512.png'));
  await render(appIcon(512, true), (0, import_node_path.join)(PWA_ICONS, 'maskable-512.png'));
  await render(appIcon(180, false), (0, import_node_path.join)(PWA_ICONS, 'apple-touch-180.png'));
  await render(appIcon(1024, false), (0, import_node_path.join)(RESOURCES, 'icon.png'));
  await render(splashImage(2732, 2732, false), (0, import_node_path.join)(RESOURCES, 'splash.png'));
  await render(
    splashImage(2732, 2732, true),
    (0, import_node_path.join)(RESOURCES, 'splash-dark.png'),
  );
