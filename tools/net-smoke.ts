...
function open(name: string) {
  ...
  return { ws, views, msgs, send, name, ready: new Promise<void>(r => { ws.onopen = () => r(); }) };
}
...
const av = A.views.at(-1)!, bv = B.views.at(-1)!;
...
console.log('A can see B cards:', Object.keys(av.cards).filter(id => bv.yourHand.includes(id) && id !== av.topCardId).length);
...
for (let i = 0; i < 300; i++) {
  const who = [A, B].find(c => {
    ...
    return v && v.status === 'PLAYING' &&
      (v.awaitingChoice ? v.awaitingChoice.playerId === v.you
                        : v.players[v.activePlayerIndex]?.id === v.you);
  });
  const v0 = A.views.at(-1)!;
  if (v0.status !== 'PLAYING') break;
  if (!who) { await new Promise(r => setTimeout(r, 30)); continue; }
  const v = who.views.at(-1)!;
  const id = `n${moves++}`;
  if (v.awaitingChoice?.type === 'COLOR' || v.awaitingChoice?.type === 'DRAW_COLOR') {
    who.send({ t: 'COMMAND', ackSeq: v.seq, command: { commandId: id, playerId: v.you, type: 'CHOOSE_COLOR', color: v.awaitingChoice.eligibleColors![0]! } });
  } else if (v.clashPending === v.you) {
    who.send({ t: 'COMMAND', ackSeq: v.seq, command: { commandId: id, playerId: v.you, type: 'CALL_CLASH' } });
  } else if (v.legalCardIds.length) {
    who.send({ t: 'COMMAND', ackSeq: v.seq, command: { commandId: id, playerId: v.you, type: 'PLAY_CARD', cardId: v.legalCardIds[0]! } });
  } else if (v.mayPass) {
    who.send({ t: 'COMMAND', ackSeq: v.seq, command: { commandId: id, playerId: v.you, type: 'END_TURN' } });
  } else {
    who.send({ t: 'COMMAND', ackSeq: v.seq, command: { commandId: id, playerId: v.you, type: 'DRAW_CARD' } });
  }
  await new Promise(r => setTimeout(r, 35));
}
const fin = A.views.at(-1)!;
console.log('final status:', fin.status, '| winner:', fin.winnerId, '| moves:', moves);
console.log('rejections:', A.msgs.filter(m => m.t === 'REJECTED').length + B.msgs.filter(m => m.t === 'REJECTED').length);
